...
st.title("Métriques projet")
st.caption(
    "Compteurs **lignes cahier des charges** et **lignes de code** sur le dépôt local (hors dossiers techniques type `.venv`)."
)
...
if top_py:
    st.subheader("Top 10 fichiers Python (taille disque + lignes)")
    rows = []
    for x in top_py:
        sz = int(x.get("size_bytes") or 0)
        rows.append(
            {
                "Fichier": Path(str(x["path"])).stem,
